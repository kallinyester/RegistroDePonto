...
$trainees = $objeto->trainees_buscar();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($resultado->num_rows == 0) {
        ...
        $trainees_id = $_POST["trainees_id"] ?? null;
        $tipo = $_POST["tipo"] ?? null;
        $hora_atual = date("H:i:s");

        if (empty($trainees_id)) {
            $mensagem = "Funcionário não selecionado.";
        } else {
            $registro->set("trainees_id", $trainees_id);

            if ($tipo == "Entrada") {
                $registro->set("hora_entrada", $hora_atual);
                $mensagem = $registro->trainees_entrada();
            } elseif ($tipo == "Saída") {
                $registro->set("hora_saida", $hora_atual);
                $mensagem = $registro->trainees_saida();
            } else {
...
<body>
    <div class="container">
        ...
        <form action="" method="POST">
            ...
            <select name="trainees_id">
                <option value="">Selecione um funcionário</option>
                <?php foreach ($trainees as $trainees): ?>
                    <option value="<?= $trainees['id'] ?>">
                        <?= htmlspecialchars($trainees['nome']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <div class="botoes">
