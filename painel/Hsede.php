...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $func = new registros();
    $func->set("registro_id", $_POST['registro_id']);
...
$sql_horas = "
            SELECT f.id, f.nome, f.situacao, 
                ...
                YEAR(s.data) AS ano, s.data, s.hora_entrada, s.hora_saida, s.total_horas_dia, s.id AS registro_id
            FROM funcionarios f
            LEFT JOIN funcionarios_sede s 
                ON f.id = s.funcionarios_id
            ORDER BY f.situacao ASC, f.nome ASC, s.data ASC, s.hora_entrada ASC";
        ...
        <main class="content">
            ...
            <div class="container">
                ...
                <form id="form" method="POST">
                    ...
                    <input type="hidden" name="id" id="id" value="" hidden/>
                            ...
                            <?php
                                ...
                                if($f['hora_entrada'] != null){
                                    ...
                                    <td style="text-align: center;" onclick="voltarTopo()">
                                        <button class="btn-edit" onclick="preencherFormulario('<?= $f['registro_id'] ?>', '<?= $f['id'] ?>', '<?= addslashes($f['nome']) ?>', '<?= addslashes($f['situacao']) ?>', '<?= $f['data'] ?>', '<?= $f['hora_entrada'] ?>', '<?= $f['hora_saida'] ?>')">
